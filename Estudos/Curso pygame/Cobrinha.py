...
#Criamos um verificador para morte:
morreu = False
...
#Criamos a função responsável por aumentar a cobra
def aumenta_cobra(lista_cobra):
    for coordenadas in lista_cobra:
        pygame.draw.rect(tela,(0,255,0),(coordenadas[0],coordenadas[1],20,20))

#Função responsável por reinicar o jogo:
def reinicar_jogo():
    #Tornamos as variáveis globais
    #OBS: ISSO É UMA MERDA, SIMONE TE ENSINOU A NÃO FAZER ISSO NEM FUDENDO, MAS HOJE VC N SABE FAZER SEM SER ASSIM, VC SO SEGUIU O VIDEO DO CARA, MELHORE ISSO DEPOIS!!!!!!
    global cont, comprimento_inicial, x, y, lista_cobra, lista_cabeca,w,z,morreu
    cont = 0
    comprimento_inicial = 5
    x = largura//2 
    y = altura//2
...
while True:
    ...
    #Vamos criar a condição para caso a cobra encoste nela mesma, ela morra, verificando se há duas coordenadas identicas na nossa lista com count, caso sim, há sobreposição e ela morre
    if lista_cobra.count(lista_cabeca) == 2:
        game_over = pygame.font.SysFont('arial',20,True,False)
        mensagem = "GAME OVER! Pressione a tecla 'R' para recomeçar"
        texto_fim = game_over.render(mensagem,False,(0,0,0))
        morreu = True
        while morreu:
            #Vamos preencher a tela de branco caso morra
            tela.fill((255,255,255))
            #Criamos toda essa estrutura para finalizar o jogo caso ocorra, contudo, caso o jogadr pressione 'r', ele irá chamar uma função que reinicia o jogo
            for evente in pygame.event.get():
                if evente.type == QUIT:
                    pygame.quit()
                    exit()
                if evente.type == KEYDOWN:
                    if evente.key == K_r:
                        reinicar_jogo()
            tela.blit(texto_fim,((largura//2)-(largura//4),(altura//2)-(altura//4)))
    ...
    #Parar a música principal caso ela morra
    #Colocar um som de derrota caso ela morra
    #Colocar um som de reviver quando ela reviver com R
